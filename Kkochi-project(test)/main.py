...
# 주소창의 파라미터(sid)를 스캔하여 세션 복원 베이스 추적 가동
...
# --- 로그인 완료 화면 분기 ---
    ...
    # 💡 안전 장치: user_info가 없으면 코드 실행을 잠시 멈추고 새로고침을 유도
    ...
    # 💡 1. 세션에 현재 메뉴가 없으면 초기값 설정
    if "current_menu" not in st.session_state:
        st.session_state["current_menu"] = menu_list[0]

    # 💡 2. 사용자가 마우스로 메뉴를 클릭했을 때만 작동하는 안전한 콜백 함수
    def change_menu():
        # .get()을 사용하여 KeyError를 완벽하게 방어합니다.
        selected_val = st.session_state.get("nav_radio")
        if selected_val:
            st.session_state["current_menu"] = selected_val

    # 💡 3. 현재 메모리에 저장된 메뉴의 위치(인덱스) 계산
    current_idx = menu_list.index(st.session_state["current_menu"]) if st.session_state["current_menu"] in menu_list else 0

    # 💡 4. 위젯의 key를 'nav_radio'로 완전히 분리하여 충돌 에러 원천 차단!
    st.sidebar.radio(
        "이동할 페이지 선택", 
        menu_list, 
        index=current_idx,
