SELECT '1. [스팸 위험도 분석] 신고 횟수가 높은 순으로 정렬하여 TOP 3 스팸 번호 조회' AS '';
...
SELECT '2. [가입 통계] 연령대별 회원 수와 평균 나이 계산' AS '';
...
SELECT '3. [최신 로그] 오늘(2026년 6월 8일) 검색된 기록만 최신순으로 조회' AS '';
...
SELECT '4. [보안 점검] 비밀번호를 안전하지 않게 설정한(나이 또는 아이디를 포함하는 등) 유저 찾기' AS '';
...
SELECT '5. [검색 기록 상세] 누가(이메일), 언제(날짜), 어떤 번호(폰번호)를 검색했는지 결합 조회' AS '';
SELECT 
    S.LOG_NUM,
...
SELECT '6. [위험 알림] 유저들이 검색한 번호 중 '실제 스팸(Y)'으로 등록된 번호와 코멘트 매칭 조회' AS '';
SELECT 
    S.LOG_NUM,
    S.USER_ID,
    S.PHONE_NUM,
...
SELECT '7. [부서 간 공유 데이터] 부재중 전화(MISSING='Y') 로그 중 스팸 번호였던 내역만 필터링' AS '';
SELECT 
    S.LOG_NUM,
    S.USER_ID,
    S.PHONE_NUM,
...
SELECT '8. [미등록 번호 추출] 검색은 되었으나 아직 스팸 테이블(SPAM_TABLE)에 등록되지 않은 '클린/신종' 번호 찾기' AS '';
...
SELECT '9. [헤비 유저 발견] 전체 평균 검색 횟수보다 더 많이 검색을 이용한 열혈 이용자의 아이디와 이메일 조회' AS '';
...
SELECT '10. [스팸 기여도] 가장 많은 스팸 번호를 검색하여 잡아낸 유저 TOP 1의 아이디와 검색 횟수' AS '';
