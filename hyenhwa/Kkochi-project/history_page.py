...
def render_history_page():
    """🎯 나의 과거 면접 기록 관리 화면 (선택 시 100% 와이드 스위칭 및 삭제 기능 통합판)"""
    ...
    if st.session_state["selected_history_item"] is None:
        ...
        with list_scroller:
            ...
            # 유저가 기본 문구 외에 진짜 이력을 선택하는 순간 세션에 정보를 적재하고 리런하여 화면 전환!
    ...
    # 💡 [스위칭 로직 분기 B단계]: 유저가 시간 이력을 체크하는 순간, 목록을 숨기고 하얀 보드판 전체를 100% 꽉 채운 와이드 리포트 모드로 변신!
        ...
        # 📌 [삭제 기능 상단 결합]: 돌아가기 버튼과 삭제 팝오버를 와이드 뷰 상단에 나란히 가로 배치
        col_btn1, col_btn2 = st.columns([2.5, 1])
        ...
        with col_btn2:
            # 실수 방지를 위해 팝오버를 사용한 2중 안전 삭제 장치 구성
            with st.popover("🗑️ 이 기록 삭제", use_container_width=True):
                st.warning("⚠️ 삭제하면 영구히 복구할 수 없습니다.")
                confirm = st.checkbox("정말 삭제할까요?", key="del_confirm_wide")
                if st.button("❌ 즉시 삭제", use_container_width=True, disabled=not confirm, type="primary"):
                    try:
                        db.delete_user_interview_history(uid, selected_h["created_at"])
                        st.session_state["selected_history_item"] = None
                        st.success("🎯 이력이 완전히 삭제되었습니다.")
                        st.rerun()
